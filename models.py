...
class ChatbotInteraction(db.Model):
    ...
    user = db.relationship(
        "User",
        primaryjoin="ChatbotInteraction.users_email == User.email",
        backref="chatbot_interactions",
    )
...
class MealPlanItem(db.Model):
    ...
    meal_time = db.Column(
        db.Enum("breakfast", "lunch", "dinner", "snack"), nullable=False
    )
    ...
    user = db.relationship(
        "User",
        primaryjoin="MealPlanItem.users_email == User.email",
        backref="meal_plan_items",
    )


class MealPlanTracking(db.Model):
    ...
    meal_plan_items_id = db.Column(
        db.ForeignKey("meal_plan_items.id"), primary_key=True
    )
...
class MealPreference(db.Model):
    ...
    user = db.relationship(
        "User",
        primaryjoin="MealPreference.users_email == User.email",
        backref="meal_preferences",
    )
...
class WeightRecord(db.Model):
    ...
    user = db.relationship(
        "User",
        primaryjoin="WeightRecord.users_email == User.email",
        backref="weight_records",
    )
