    ...
    steps {
        withCredentials([usernamePassword(credentialsId: "${MANIFEST_REPO_CREDENTIALS_ID}", usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD')]) {
            script {
                ...
                sh "git clone https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/Mozo119/Jenkins_backend_manifast.git Jenkins_backend_manifest"
                ...
                sh "git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/Mozo119/Jenkins_backend_manifast.git"
            }
        }
    }
}
